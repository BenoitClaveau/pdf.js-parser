class Rect {
    ...
    static collideRectRect(x, y, w, h, x2, y2, w2, h2) {
        //2d
    ...
    static collideRectCircle(rx, ry, rw, rh, cx, cy, diameter) {
        //2d
        ...
        const distance = Rect.dist(cx, cy, testX, testY)
    ...
    static collideCircleCircle(x, y, d, x2, y2, d2) {
        //2d
        if (Rect.dist(x, y, x2, y2) <= (d / 2) + (d2 / 2)) {
            return true;
        }
        return false;
    };

    static collidePointCircle(x, y, cx, cy, d) {
        //2d
        if (Rect.dist(x, y, cx, cy) <= d / 2) {
            return true;
        }
        return false;
    };

    static collidePointEllipse(x, y, cx, cy, dx, dy) {
        //2d
        const rx = dx / 2;
        const ry = dy / 2;
    ...
    static collidePointRect(pointX, pointY, x, y, xW, yW) {
        //2d
    ...
    static collidePointLine(px, py, x1, y1, x2, y2, buffer) {
        // get distance from the point to the two ends of the line
        const d1 = Rect.dist(px, py, x1, y1);
        const d2 = Rect.dist(px, py, x2, y2);

        // get the length of the line
        const lineLen = Rect.dist(x1, y1, x2, y2);
    ...
    static collideLineCircle(x1, y1, x2, y2, cx, cy, diameter) {
    ...
    static collideLineLine(x1, y1, x2, y2, x3, y3, x4, y4, calcIntersection) {
    ...
    static collideLineRect(x1, y1, x2, y2, rx, ry, rw, rh, calcIntersection) {
    ...
    static collidePointPoly(px, py, vertices) {
        let collision = false;
    ...
    // POLYGON/CIRCLE
    static collideCirclePoly(cx, cy, diameter, vertices, interior) {
    ...
    static collideRectPoly(rx, ry, rw, rh, vertices, interior) {
        if (interior == undefined) {
            interior = false;
        }
    ...
    static collideLinePoly(x1, y1, x2, y2, vertices) {
    ...
    static collidePolyPoly(p1, p2, interior) {
        if (interior == undefined) {
            interior = false;
        }
        ...
        for (let current = 0; current < p1.length; current++) {
            ...
            const collision = this.collideLinePoly(vc.x, vc.y, vn.x, vn.y, p2);
            if (collision) return true;
    ...
    static collidePointTriangle(px, py, x1, y1, x2, y2, x3, y3) {
    ...
    static collidePointPoint(x, y, x2, y2, buffer) {
        if (buffer == undefined) {
            buffer = 0;
        }

        if (Rect.dist(x, y, x2, y2) <= buffer) {
            return true;
        }
    ...
    static collidePointArc(px, py, ax, ay, arcRadius, arcHeading, arcAngle, buffer) {
    ...
    static dist() {
        if (arguments.length === 4) {
            return Rect.hypot(arguments[2] - arguments[0], arguments[3] - arguments[1]);
    ...
    static insideRectRect(x, y, w, h, x2, y2, w2, h2) {
        const res = x >= x2 && x + w <= x2 + w2 && y >= y2 && y + h <= y2 + h2;
        return res;
    }

    static equalsRectRect(x, y, w, h, x2, y2, w2, h2, distance = 0) {
        return Math.abs(x - x2) < distance && 
                Math.abs(y - y2) < distance && 
                Math.abs(w - w2) < distance * 2 && 
                Math.abs(h - h2) < distance * 2; 
    }

    static equalsRectObj(rect1, rect2, distance = 0) {
        return Math.abs(rect1.x - rect2.x) < distance && 
                Math.abs(rect1.y - rect2.y) < distance && 
                Math.abs(rect1.w - rect2.w) < distance * 2 && 
                Math.abs(rect1.h - rect2.h) < distance * 2; 
    }

    static intersectRect(x, y, w, h, x2, y2, w2, h2) {

        const collides1 = Rect.collideRectRect(x, y, w, h, x2, y2, w2, h2);
        const collides2 = Rect.collideRectRect(x2, y2, w2, h2, x, y, w, h);
        const inside1 = Rect.insideRectRect(x, y, w, h, x2, y2, w2, h2);
        const inside2 = Rect.insideRectRect(x2, y2, w2, h2, x, y, w, h);
        
        if (!collides1 && !collides2 && !inside1 && !inside2) 
            return null;

        const top = Math.max(y, y2);
        const bottom = Math.min(y + h, y2 + h2);
        const left = Math.max(x, x2);
        const right = Math.min(x + w, x2 + w2);
        return {
            x: left,
            y: top,
            w: right - left,
    ...
    static unionRect(x, y, w, h, x2, y2, w2, h2) {

        const top = Math.min(y, y2);
        const bottom = Math.max(y + h, y2 + h2);
        const left = Math.min(x, x2);
        const right = Math.max(x + w, x2 + w2);
        return {
            x: left,
            y: top,
            w: right - left,
    ...
    static unionRects(rects) {
        ...
        for (let i = 1; i < rects.length; i++) {
            const r1 = prev ? prev : rects[0];
            const r2 = rects[i];
            prev = Rect.unionRect(r1.x, r1.y, r1.w, r1.h, r2.x, r2.y, r2.w, r2.h);
        }
        
        return prev;
    }

    static inflateRect(x, y, w, h, xscl, yscl = xscl) {
        const xAdj = (w * xscl - w) / 2;
        let yAdj = (h * yscl - h) / 2;
    ...
    static inflateFixedRect(x, y, w, h, xfixed, yfixed = xfixed) {
        return {
            x: x - xfixed,
            y: y - yfixed,
            w: w + (xfixed * 2),
